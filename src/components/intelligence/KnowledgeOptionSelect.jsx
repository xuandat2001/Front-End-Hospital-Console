...
function KnowledgeOptionSelect({ value, options, onChange, disabled = false }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-[80] max-h-[180px] overflow-y-auto rounded-xl border border-white/10 bg-slate-900 py-1 shadow-2xl scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-9 w-full items-center px-3 text-left text-sm transition ${
                  isSelected
